...
if ( props.list.folders && props.list.folders.length>0 ) {
  folders = ( <div className={style.card_container}>
      {Object.values(props.list.folders).map((folder,index) => {
        
        return <div className={style.cardWrapper}>
          {index===0 && props.movingState===1 ? <div className={style.emptySpaceBefore}><AddCircleIcon /></div> : "" }
          {index===0 && props.movingState===0 ? <div className={style.emptySpaceBeforeUnactive}></div> : "" }
          {props.movingState===1 ? <div className={style.overlayMove}><AddCircleIcon /></div>: "" }
          <MediaCard 
          title={folder.title} 
          ...
          />{props.movingState===1 ? <div className={style.emptySpaceAfter}><AddCircleIcon /></div> : <div className={style.emptySpaceAfterUnactive}></div>}</div>
      })}
   </div>
  );
}
...
if ( props.list.modules && props.list.modules.length>0 ) {
  ...
  modules = ( <div className={style.card_container}>
